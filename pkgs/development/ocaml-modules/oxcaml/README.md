...
Checked 2026-09-06. **Nothing these pins depend on had moved**, so none of the
patches above could be dropped as fixed upstream:
...
* All 141 Jane Street pins in `../janestreet/0.18-oxcaml.nix` are unmoved: 140
  ...
  which has none, is still at the v0.15.1 commit oxopam names. No repo has a
  `v0.18` branch or tag at all. Jane Street pushes these branches as one batch
  export from its monorepo, so they move together or not at all; the last
  export is still the 2026-07-10 one.
* oxopam's `main` is three commits ahead of the pin (last commit 2026-08-31),
  but none of the three touch any of the 20 patch directories named in
  ./patch-sets.nix: they are `oxcaml-*.guard` bookkeeping, dune/eio/mdx version
  bounds, and `wasm_of_ocaml-compiler`, a different package from the
  `js_of_ocaml-compiler` set the wasm patches here come from. Its newest Jane
  Street set is still the pinned `v0.18~preview.130.106+341`, so moving the pin
  would rebuild every patched derivation for no change in behaviour. Of the
  other branches, `unified` is strictly behind the pin; `dev` and
  `5.2.0minus38` have diverged from it rather than advanced past it.
